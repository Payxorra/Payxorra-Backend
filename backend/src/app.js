...
app.post("/api/admin/consolidate-accounts", async (req, res) => {
  try {
    ...
    if (!primaryAddress || !Array.isArray(addressesToMerge) || addressesToMerge.length === 0 || !adminAddress) {
      return res.status(400).json({
        success: false,
        error: "primaryAddress, addressesToMerge (non-empty array), and adminAddress are required"
      });
    }
